...
(Prisma). The active schema is `packages/database/prisma/schema/` and covers files and users so far.
`packages/database/prisma/drafts/schema/` holds an older draft covering courses, wallets, offers and
student lists — it is a draft, not the active schema, and is ported model by model as tasks need it.
